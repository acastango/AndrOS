...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.ANDROS_NO_RESPAWN) || isTruthyEnvValue(process.env.OPENCLAW_NO_RESPAWN)) return false;
  if (isTruthyEnvValue(process.env.ANDROS_NODE_OPTIONS_READY) || isTruthyEnvValue(process.env.OPENCLAW_NODE_OPTIONS_READY)) return false;
  const nodeOptions = process.env.NODE_OPTIONS ?? "";
  if (hasExperimentalWarningSuppressed(nodeOptions)) return false;

  process.env.ANDROS_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[andros] ${parsed.error}`);
    process.exit(2);
  }
