...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.androsCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.androsCanvasA2UIAction &&
        typeof window.androsCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.androsSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("andros:a2ui-action-status", onStatus);
  ...
  function send(name, sourceComponentId) {
    ...
    const sendUserAction =
      typeof window.androsSendUserAction === "function"
        ? window.androsSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ANDROS_SKIP_CANVAS_HOST)) return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".andros", "canvas")];
