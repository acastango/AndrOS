...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ANDROS_PROFILE?.trim() || env.OPENCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(homedir(), ".andros", `workspace-${profile}`);
  }
  return path.join(homedir(), ".andros", "workspace");
}
